...
public class ReportsController {
    ...
    private void setupActionColumn() {
        colActions.setCellFactory(param -> new TableCell<>() {
            ...
            private final HBox   container = new HBox(10, btnView, btnDelete);
            {
                ...
                // View opens the preview with all stored fields restored
    ...
    /** Opens the preview for a previously saved report, restoring all stored header fields. */
    private void openPreviewForSaved(Report report) {
        try {
            ...
            ctrl.loadSavedReport(report);   // ← uses the new path that reads Report fields
    ...
    public void downloadReports(ActionEvent event) {
        ...
        if (selected.isEmpty()) {
            showAlert(Alert.AlertType.WARNING, "Nothing selected",
                    "Please check at least one report.");
            return;
        }
        // Open the preview for each selected report (user can then export from there)
